...
class World(object):
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            allpos = list((x, y) for x in range(SNAKE_SIZE + 1, self.size[0] - SNAKE_SIZE - 1)  # 2(world)
                          for y in range(SNAKE_SIZE + 1, self.size[1] - SNAKE_SIZE - 1))  # 2(world)
            start_position = allpos[np.random.randint(len(allpos))]
    ...
    def init_food(self):
        ...
        available_food_positions = list((x, y) for x in range(1, self.size[0]-1) for y in range(1, self.size[1]-1))
        for i in snake.blocks:                                                                              #5world
            if i in available_food_positions:                                                               #5world
                available_food_positions.remove(i)                                                          #5world
        if not self.custom:
            # Choose a random position from available
            chosen_position = available_food_positions[np.random.randint(len(list(available_food_positions)))] #6world
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            # Check if snake eats itself
                    ...
                    or new_snake_head==old_snake_tail:
                self.snake.alive = False
